...
class XBeeGUI:
    def __init__(self, root):
        ...
        self.square_button = tk.Button(self.move_frame, text="Square", height=2, width=15, 
                                        command=self.send_square, bg="#0ff5f5")
        self.square_button.grid(row=5, column=2, columnspan=3, pady=2)

        self.return_control_button = tk.Button(self.move_frame, text="Return Control", height=2, width=15, 
                                        command=self.return_control, bg="#0ff5f5")
        self.return_control_button.grid(row=5, column=4, columnspan=3, pady=2)

        self.battery_status_entry = tk.Entry(self.move_frame, width=20)
        self.battery_status_entry.grid(row=6, column=2, pady=2, padx=5)
            ...
            try:
                ...
                if text.startswith("BATT "):
                    ...
                    if len(parts) >= 2 and parts[1].endswith("V"):
                        ...
                        self.battery_status_entry.insert(0, voltage)
                # Check for battery status error
